...
// N8nPromptService now requires PrismaService — pass null for test (resolveNodeToSubType won't be called with explicit subType)
const service = new N8nPromptService(null as any);

async function runTests() {
  console.log('=== TEST 1: QNA Prompt ===');
  const qnaPrompt = await service.generatePrompt({ type: 'QNA', title: 'Kenapa kita harus sholat?' });
  ...
  const kisahPrompt = await service.generatePrompt({ type: 'KISAH', title: 'Hijrah ke Madinah', subType: 'SIRAH' });
  ...
  const fiksiPrompt = await service.generatePrompt({ type: 'KISAH', title: 'Petualangan di Taman', subType: 'FIKSI' });
  ...
  const articlePrompt = await service.generatePrompt({ type: 'ARTICLE', title: 'Pentingnya Sedekah' });
  ...
  const pembelajaranPrompt = await service.generatePrompt({ type: 'PEMBELAJARAN', title: 'Rukun Islam' });
