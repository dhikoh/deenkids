  ...
  const renderBlockEditor = (block: EditorBlock, index: number) => {
    ...
    return (
      <div key={id} className="relative bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden group">
        ...
        <div className="p-4">
          {type === "paragraph" && <div className="space-y-2">{data.heading !== undefined && <input type="text" value={data.heading || ''} onChange={e => updateBlock(id, { heading: e.target.value })} placeholder="📋 Sub-judul babak (opsional — tidak dibaca TTS)" className="w-full border-slate-200 rounded-lg p-2 text-sm font-bold text-slate-800 bg-amber-50/50 placeholder:font-normal" />}<textarea value={data.text} onChange={e => updateBlock(id, { text: e.target.value })} placeholder="Tulis isi konten..." className="w-full border-slate-200 rounded-lg p-3 min-h-[100px] focus:border-emerald-500 focus:ring-emerald-500" /><input type="url" value={data.referenceUrl || ''} onChange={e => updateBlock(id, { referenceUrl: e.target.value })} placeholder="📎 Link referensi (opsional) — https://..." className="w-full border-slate-200 rounded-lg p-2 text-sm text-emerald-700" /></div>}
