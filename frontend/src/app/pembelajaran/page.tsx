...
import { BookOpen, ChevronRight, Star, Layers, Search, ArrowRight } from "lucide-react";
...
export default function PembelajaranPage() {
  ...
  const handleSearch = () => {
    if (query.trim().length >= 2) {
      router.push(`/search?q=${encodeURIComponent(query.trim())}&type=PEMBELAJARAN`);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12 pt-28">
      ...
      {/* Search Bar */}
      <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex gap-2 mb-10 max-w-2xl mx-auto">
        ...
        <button
          ...
          className="h-10 px-5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 flex items-center gap-2 text-sm transition-colors"
        >
            ...
            {category.children && category.children.length > 0 && (
              <div className="p-4 space-y-1">
                {category.children.map((mod: any) => (
                  <Link key={mod.id} href={`/pembelajaran/${mod.slug}`} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 text-sm font-medium text-slate-600 hover:text-emerald-600 transition-colors group/link">
                    <span>{mod.title}</span>
